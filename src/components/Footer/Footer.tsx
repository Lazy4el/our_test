    ...
    <Box sx={{ flexGrow: 1, marginTop: '20px' }}>
        ...
        <Grid  item xs={4} md={6} sx={{textAlign:{md:'start', xs:'initial'}}}>
            <Link to='/'  className={`${s.logoImg} ${s.logo}`} >
                    ...
                    <span className={`logo-general-span ${s.logoSpan}`}>НефтеХим<span className='logo-ink-span'>Полимер</span></span>
            </Link>
            <div className={s.spanContainer}>
            <span className={s.text}> ООО «НефтеХимПолимер» Copyright © 2018.</span><span className={s.lineBreak}>Все права защищены. 
        ...
        <ul className={s.list}>
        {
                        headerLinks.map((el: { key: React.Key | null | undefined; link: To; label: string | number | boolean | React.ReactElement<any, string | React.JSXElementConstructor<any>> | React.ReactFragment | null | undefined; }) => {
                                 return (
                                        <Link
